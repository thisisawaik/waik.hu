...
export class DiscordEmbedComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private msg: MessagesService) {}

  ngOnInit(): void {
    this.http
      .get('https://discord.com/api/guilds/541446521313296385/widget.json')
      .toPromise()
      .then((res: any) => {
  ...
  join() {
    ...
    if (this.invite) {
      const url =
        `${
          this.invite.split('?')[0]
        }?utm_source=waik.hu&utm_medium=join_button` ||
        'https://discord.com/invite/RA8uasa?utm_source=waik.hu&utm_medium=join_button';
